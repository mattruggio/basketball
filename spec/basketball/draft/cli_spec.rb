...
describe Basketball::Draft::CLI do
  describe 'feature tests' do
    ...
    context 'with no fuzz and no depth' do
      let(:dir) { File.join('draft', 'no_fuzz_no_depth_draft') }
      ...
      specify 'sim 2 picks' do
        output_path   = File.join(TEMP_DIR, dir, "#{prefix}1-2.json")
      ...
      specify 'sim 3 picks' do
        output_path   = File.join(TEMP_DIR, dir, "#{prefix}1-3.json")
      ...
      specify 'sim 4 picks' do
        output_path   = File.join(TEMP_DIR, dir, "#{prefix}2-1.json")
        args          = ['-i', input_path, '-o', output_path, '-s', '3']
